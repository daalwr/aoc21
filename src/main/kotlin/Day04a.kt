...
fun main(args: Array<String>) {
    ...
    for (bingoCardNumber in 0 until bingoCardCount) {
        ...
        val bingoCard = BingoCard(bingoCardLines.toList())
        bingoCards.add(bingoCard)
    }

    for (i in 1 until allWinningNumbers.size) {
        val chosenNumbers = allWinningNumbers.take(i)

        for (bingoCard in bingoCards) {
            if (bingoCard.isWinner(chosenNumbers)) {
                val sumOfUnmarkedNumbers = bingoCard.sumOfUnmarkedNumbers(chosenNumbers)
