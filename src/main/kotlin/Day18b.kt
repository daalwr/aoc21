...
//                println(input)
    ...
    for (x in lines.indices) {
        for (y in lines.indices) {
            if (x != y) {
                val line1 = parse(lines[x])
                val line2 = parse(lines[y])

                val lastInCurrent = getLast(line1)
                val firstInNext = getFirst(line2)

                lastInCurrent.next = firstInNext
                firstInNext.prev = lastInCurrent

                val sum = SnailFishPair(1, line1, line2, null)
                sum.left.parent = sum
                sum.right.parent = sum

                setDepths(sum)
                reduce(sum)
                val magnitude = calculateMagnitude(sum)
